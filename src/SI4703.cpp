///
/// \file SI4703.cpp
/// \brief Implementation for the radio library to control the SI4703 radio chip for STM32duino.\n
/// \author Felice Murolo\n
/// \email linuxboy@giove.tk\n
/// \copyright Copyright (c) 2019 by Felice Murolo, Salerno, Italia\n
/// \original_author Matthias Hertel, http://www.mathertel.de
/// \original_copyright Copyright (c) 2014-2015 by Matthias Hertel.\n
...
/// --------
/// * 05.08.2014 created.
/// * 24.03.2019 reworked and modified by Felice Murolo
...
// WIRING FOR STM32duino
#define LED PC13
#define EXT_LED PB3 // connect this pin to a 120 Ohm resistor + one led
#define resetPin PB4
#define SDIO PB7
#define SCLK PB6
...
void SI4703::getRadioInfo(RADIO_INFO *info) {
  ...
  if (registers[STATUSRSSI] & (RDSR)) info->rds = true;
  if (registers[STATUSRSSI] & STC) info->tuned = true;
  if (registers[POWERCFG] & (1 << SETMONO)) info->mono = true;

  if (info->stereo == true) {
      digitalWrite(LED,LOW); // put LED ON
      digitalWrite(EXT_LED,HIGH); // put EXT_LED ON
  }
  else {
      digitalWrite(LED,HIGH); // put LED OFF
      digitalWrite(EXT_LED,LOW); // put EXT_LED OFF
  }
} // getRadioInfo()
